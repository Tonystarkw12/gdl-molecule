...
本项目实现了一个基于 **SchNet** (Schütt et al., 2018) 的几何深度学习模型，用于预测分子的量子化学性质。该模型专门设计用于处理分子的 3D 几何信息，是向 AI 驱动分子动力学领域专家展示技术能力的理想项目。
...
**面试官提问："为什么选择 SchNet 而非普通 GCN？"**

> "传统 GCN 只利用分子的图结构（原子连接关系），但分子的物理化学性质（如 HOMO-LUMO gap）高度依赖于原子的空间位置。
>
> SchNet 通过**连续滤波卷积**直接对原子间的 3D 距离建模：
> 1. 使用高斯 RBF 将距离扩展为高维特征
> 2. 通过可学习的滤波网络生成距离依赖的权重
> 3. 滤波器是连续函数，能平滑处理任意距离值
>
> 这样模型既能学习局域的键相互作用，也能捕捉长程的非键相互作用（如氢键），对分子动力学模拟至关重要。"
...
- ✅ 高斯 RBF 的物理意义
- ✅ 为什么 3D 信息对分子性质重要
- ✅ Message Passing 的数学形式
- ✅ SchNet 与其他 GNN (GCN/GAT/GraphSAGE) 的区别

### 可能的追问
1. "如何处理变长分子？" → 全局池化
2. "为什么用加法聚合？" → 物理量可加性（能量等）
3. "如何捕捉旋转不变性？" → 距离是旋转不变的
4. "如果用角度信息会怎样？" → 更复杂的几何特征
